...
const { registerValidator, checkUser, loginValidator } = require('./validNewUser');
const { validationResult, check } = require('express-validator');
const { first } = require('../database/dbConfig');


// POST /auth/register new user
router.post('/register', registerValidator(), checkUser, (req, res) => {
  ...
  Users.addUser(user)
    .then(u => {
      ...
      res.status(201).json({ message: `Welcome ${u.username}`, user: u, token })
    ...
    .then(u => {
      if (u) {
        Users.removeUser(u.id)
          .then(u => {
            res.status(201).json({ message: `User with 🆔 ${deletedUser} is deleted from the database.`})
          }).catch(err => {console.log(err); res.status(500).json({ err: `Error deleting user id ${deletedUser}`})});
      } else {
        res.status(400).json({ message: `No user with 🆔 ${deletedUser} in database..` })
      }
